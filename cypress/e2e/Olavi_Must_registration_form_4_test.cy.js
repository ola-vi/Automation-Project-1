...
describe('Input fields', () => {
    ...
    it('Username should support only underscore, letters and numbers', () => {
        // check with regex supporter format
        cy.get('#username').should('have.attr', 'pattern', '[a-zA-Z0-9_]+')     // underscore is not ok if it needs to match only numbers and letters but its same in HTML so for passing it needs to be like that. So changing title that is wrong
    })
    ...
    it('Email input should support correct pattern', () => {          
       ...
       // cy.get('#email').should('have.attr', 'pattern').should('contain', '[a-z0-9]+@[a-z0-9]+\\.[a-z]{2,4}$')
        cy.get('#email').should('have.attr', 'pattern').should('equal', '[a-z0-9]+@[a-z0-9]+\\.[a-z]{2,4}$')
